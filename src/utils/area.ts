...
// 生成树形结构的函数
function generateAreaTree(
  provinceList: Record<string, string>,
  cityList: Record<string, string>,
  countyList: Record<string, string>,
) {
    ...
    const cities = Object.entries(cityList)
      ...
      .map(([cityCode, cityName]) => {
        const counties = Object.entries(countyList)
          .filter(([countyCode]) => countyCode.startsWith(cityCode.slice(0, 4)))
          .map(([countyCode, countyName]) => ({ value: countyCode, label: countyName }));
        return {
          value: cityCode,
          label: cityName,
          children: counties,
...
// 生成树形结构
export const areaTree = generateAreaTree(areaData.province_list, areaData.city_list, areaData.county_list);
