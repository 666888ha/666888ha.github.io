...
import { customerApproval, getEmployeeList } from '@/api/customer/customer';
...
// 当前审批的客户ID
const currentCustomerId = ref<string | number | null>(null);
...
export type ApproveSuccessDialogShowOptions = {
  /** 打开时默认勾选的通知人（如客户归属人员 user id） */
  defaultNotifyUserIds?: Array<string | number>;
};

// 显示弹框
const show = async (customerId: string | number, options?: ApproveSuccessDialogShowOptions) => {
...
const handleConfirm = async () => {
  ...
  if (!currentCustomerId.value) {
    MessagePlugin.error('缺少客户ID，无法提交审批');
    return;
  }
  ...
  try {
    const res = await customerApproval({
      id: currentCustomerId.value,
