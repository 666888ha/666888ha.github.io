<template>
  <div class="work-report-detail-container">
    ...
    <t-card :bordered="false" class="header-card">
      <div class="header-content">
        ...
        <div class="header-right">
          <t-button v-if="props.isApprove" theme="primary" @click="handleApprove">
            <template #icon>
              <t-icon name="edit" />
            </template>
            点评报告
    ...
    <!-- 点评报告弹框 -->
    <approve-comment-dialog ref="approveCommentDialogRef" @success="handleApproveSuccess" />
  </div>
</template>
...
import { getWorkReportDetail } from '@/api/workReport';
...
const props = withDefaults(
  ...
  {
    isApprove: false, // 默认为详情页面
  },
);
const ApproveCommentDialog = defineAsyncComponent(() => import('../components/approveCommentDialog.vue'));
...
// 点评弹框引用
const approveCommentDialogRef = ref<InstanceType<typeof ApproveCommentDialog> | null>(null);
...
const handleApprove = () => {
  ...
  if (approveCommentDialogRef.value) {
    approveCommentDialogRef.value.show(id);
  }
};
...
const handleApproveSuccess = () => {
  // 返回列表页
  router.push('/approveMange/workReportApprove');
};
