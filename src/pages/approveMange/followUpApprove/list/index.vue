      ...
      >
        ...
        <template #customer_name="{ row }">
          <div class="customer-name-container">
            <t-tooltip :content="String(row?.customer?.customer_name ?? '-')" placement="top">
        ...
        <template #operation="{ row }">
          <t-space>
            ...
            <t-link v-if="!row.approval_status" theme="primary" @click="clickOper(11, row)"> 审批 </t-link>
          </t-space>
        </template>
      </t-table>
    </t-card>
...
const clickOper = async (type: number, row) => {
  switch (type) {
    ...
    case 11: // 审批
      router.push({
        path: '/approveMange/followUpApprove/approve',
        query: { id: row.id },
      });
      break;
    case 12: // 跟进人详情
