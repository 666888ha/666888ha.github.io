    ...
    <t-tabs v-model="activeTab" class="report-tabs" @change="handleMainTabChange">
      <t-tab-panel value="submitted" label="提交给我的">
        ...
        <div v-if="activeTab === 'submitted'">
          ...
          <t-card :bordered="false" class="table-card">
            <t-table :data="tableData" :columns="tableColumns" :row-key="rowKey" :loading="tableLoading" :hover="true">
              ...
              <template #operation="{ row }">
                <t-button theme="primary" variant="text" @click="handleViewDetail(row)"> 详情 </t-button>
          ...
          <!-- 添加工作报告弹框 -->
          <add-work-report-dialog ref="addWorkReportDialogRef" @success="handleAddSuccess" />
...
// 添加工作报告弹框引用
const addWorkReportDialogRef = ref<InstanceType<typeof AddWorkReportDialog> | null>(null);

// 添加
const handleAdd = () => {
  if (addWorkReportDialogRef.value) {
    addWorkReportDialogRef.value.show();
  }
};
...
const handleViewDetail = (row: any) => {
  router.push(`/workReport/workReportDetail?id=${row.id}`);
};
