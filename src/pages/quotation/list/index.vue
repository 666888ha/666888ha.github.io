<template>
  <div class="customer-container">
    ...
    <t-tabs v-model="activeTab" class="customer-tabs" @change="handleTabChange">
      <t-tab-panel value="all" label="全部报价"> </t-tab-panel>
      <t-tab-panel value="self" label="我的报价"> </t-tab-panel>
      <t-tab-panel value="subordinate" label="下属报价"> </t-tab-panel>
    ...
    <div class="operation-bar">
      <div class="operation-left">
        <t-button theme="primary" @click="handleAddQuotation">
          <template #icon>
            <t-icon name="add" />
          </template>
          添加报价
...
import { deleteCustomerQuotation, generateQuotationPDF, getQuotationList } from '@/api/quotation';
...
// 生成Word报价单
const handleGeneratePDF = async (row: QuotationData) => {
  // 获取报价ID
  if (!row.id) {
    MessagePlugin.warning('请先保存报价后再生成Word报价单');
    return;
  }

  try {
    MessagePlugin.loading('正在生成Word报价单...');
    const response = await generateQuotationPDF(row.id);

    if (response && (response.code === 0 || response.code === 200)) {
      MessagePlugin.closeAll();
      MessagePlugin.success(response.msg || 'Word报价单生成成功');
    } else {
      MessagePlugin.closeAll();
      MessagePlugin.error(response?.msg || 'Word报价单生成失败');
    }
  } catch (error: any) {
    MessagePlugin.closeAll();
    console.error('生成Word报价单失败:', error);
    MessagePlugin.error(error?.response?.data?.msg || error?.message || '生成Word报价单失败，请重试');
  }
};
// 选择变化
...
const loadTableData = async () => {
  ...
  try {
    ...
    if (
      searchForm.value.approval_status !== '' &&
      searchForm.value.approval_status !== null &&
      searchForm.value.approval_status !== undefined
    ) {
