    ...
    <t-card :bordered="false" class="form-card">
      <t-form ref="formRef" :data="formData" :rules="formRules" :label-width="100" @submit="handleSubmit">
        <t-row :gutter="[24, 24]">
          <!-- 物品编码 -->
          <t-col :span="6">
            <t-form-item label="物品编码" name="code" required>
              <t-input v-model="formData.code" placeholder="请输入物品编码" />
            </t-form-item>
          </t-col>

          <!-- 物品名称 -->
          <t-col :span="6">
            <t-form-item label="物品名称" name="name" required>
              <t-input v-model="formData.name" placeholder="请输入物品名称" />
            </t-form-item>
          </t-col>

          <!-- 物品分类 -->
          <t-col :span="6">
            <t-form-item label="物品分类" name="category" required>
              <t-select v-model="formData.category" :options="categoryOptions" placeholder="请选择物品分类" clearable />
            </t-form-item>
          </t-col>

          <!-- 物品属性 -->
          <t-col :span="6">
            <t-form-item label="物品属性" name="attribute" required>
              <t-select v-model="formData.attribute" :options="attributeOptions" placeholder="请选择物品属性" clearable />
            </t-form-item>
          </t-col>

          <!-- 物品位置 -->
          <t-col :span="6">
            <t-form-item label="物品位置" name="location" required>
              <t-input v-model="formData.location" placeholder="请输入物品位置" />
            </t-form-item>
          </t-col>

          <!-- 物库存数量 -->
          <t-col :span="6">
            <t-form-item label="物库存数量" name="stock" required>
              <t-input-number v-model="formData.stock" :min="0" theme="column" placeholder="请输入库存数量" />
            </t-form-item>
          </t-col>
...
const formRules: FormRules<typeof formData> = {
  ...
  category: [{ required: true, message: '请选择物品分类', type: 'error', trigger: 'change' }],
  attribute: [{ required: true, message: '请选择物品属性', type: 'error', trigger: 'change' }],
  location: [{ required: true, message: '请输入物品位置', type: 'error', trigger: 'blur' }],
  stock: [{ required: true, message: '请输入库存数量', type: 'error', trigger: 'blur' }],
};
...
const loadDetail = async () => {
  ...
  try {
    ...
    if (res.code === 0 || res.code === 200) {
      ...
      // 映射API返回的字段到表单字段
      formData.code = detail.goods_code || '';
      formData.name = detail.goods_name || '';
      formData.category = detail.goods_cate || '';
      formData.attribute = detail.goods_attr || '';
      formData.location = detail.goods_location || '';
      formData.stock = Number(detail.stock_num) || 0;
...
const handleSubmit = async ({ validateResult, firstError }: SubmitContext) => {
  ...
  try {
    // 组装提交参数（按照API接口参数格式）
    const payload: any = {
      goods_code: formData.code.trim(),
      goods_name: formData.name.trim(),
      goods_cate: formData.category,
      stock_num: String(formData.stock), // 转为字符串
      goods_attr: formData.attribute,
      goods_location: formData.location.trim(),
      goods_img: formatAttachments(formData.attachments || []), // 数组格式
    ...
    if (response.code === 0 || response.code === 200) {
      ...
      MessagePlugin.error(response.msg || response.message || (props.mode === 'edit' ? '编辑物品失败' : '新增物品失败'));
    }
  } catch (error: any) {
