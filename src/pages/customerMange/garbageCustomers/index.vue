<template>
  <div class="customer-container">
    ...
    <custom-column-dialog
      ...
      custom-key-name="garbageCustomerKey"
...
import { batchReceiveGarbage, deleteCustomer, getGarbageList } from '@/api/customer/customer';
...
const initTableColumns = (): PrimaryTableCol[] => {
  const savedConfig = localStorage.getItem('garbageCustomerKey');
  if (savedConfig) {
    try {
...
const loadTableData = async () => {
  ...
  try {
    ...
    const response = await getGarbageList(params);
...
const showClaimCustomerDialog = () => {
  const confirmDialog = DialogPlugin.confirm({
    ...
    body: () =>
      h('div', { class: 'claim-customer-dialog-content' }, [
        ...
        h('div', { class: 'dialog-text-content' }, [
          ...
          h('div', { class: 'info-text' }, '领取后14天后未跟进/成交,客户将自动放回公海'),
        ]),
      ]),
    confirmBtn: '确定',
    cancelBtn: '取消',
    ...
    onConfirm: async ({ e }) => {
      try {
        const response = await batchReceiveGarbage(selectedRowKeys.value);
        if (response.code === 0 || response.code === 200) {
