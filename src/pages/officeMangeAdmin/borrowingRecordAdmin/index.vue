      ...
      >
        ...
        <template #operation="{ row }">
          <t-space>
            ...
            <t-link v-if="row.audit_status === 0" theme="primary" @click="clickOperate(3, row)">撤销</t-link>
            <t-link v-if="row.record_type === 2 && row.audit_status === 1" theme="primary" @click="clickOperate(4, row)"
              >归还</t-link
            >
            <t-link v-if="row.record_type === 2 && row.audit_status === 1" theme="primary" @click="clickOperate(5, row)"
              >续借</t-link
            >
          </t-space>
        </template>
...
import { cancelOfficeGoods, getOfficeGoodsRecordList, renewOfficeGoods, returnOfficeGoods } from '@/api/office';
...
const tableColumns: PrimaryTableCol[] = [
  ...
  { colKey: 'status', title: '物品状态', width: 120 },
  { colKey: 'audit_status', title: '审核状态', width: 120 },
...
const clickOperate = async (type: number, row: any) => {
  switch (type) {
    case 1:
    case 2:
      break;
    case 3: {
      ...
      const confirmDia = DialogPlugin.confirm({
        header: '撤销操作',
        body: `确定要撤销物品【${row.goods_name || ''}】的操作吗？`,
        theme: 'warning',
        confirmBtn: '确定',
        cancelBtn: '取消',
        onConfirm: async () => {
          try {
            const res = await cancelOfficeGoods({ record_id: row.id });
            if (res.code === 0 || res.code === 200) {
              MessagePlugin.success(res.msg || '撤销成功');
              loadTableData();
            } else {
              MessagePlugin.error(res.msg || '撤销失败');
            }
          } catch (error: any) {
            console.error('撤销操作失败:', error);
            MessagePlugin.error(error?.response?.data?.msg || error?.message || '撤销失败，请重试');
          } finally {
            confirmDia.hide();
          }
        },
    ...
    case 5: {
      // 续借操作
      const renewConfirm = DialogPlugin.confirm({
        header: '续借操作',
        body: `确定要续借物品【${row.goods_name || ''}】吗？`,
        theme: 'warning',
        confirmBtn: '确定',
        cancelBtn: '取消',
        onConfirm: async () => {
          try {
            const res = await renewOfficeGoods({ original_record_id: row.id });
            if (res.code === 0 || res.code === 200) {
              MessagePlugin.success(res.msg || '续借成功');
              loadTableData();
            } else {
              MessagePlugin.error(res.msg || '续借失败');
            }
          } catch (error: any) {
            console.error('续借操作失败:', error);
            MessagePlugin.error(error?.response?.data?.msg || error?.message || '续借失败，请重试');
          } finally {
            renewConfirm.hide();
          }
        },
