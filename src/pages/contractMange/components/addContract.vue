<template>
  <div class="contract-add-container">
          ...
          <t-col :span="6">
            <t-form-item label="关联报价" required-mark>
              <div class="product-select-wrapper">
                <t-button theme="primary" variant="outline" @click="showProductDialog">
                  <template #icon>
                    <t-icon name="add" />
                  </template>
                  选择报价单
                </t-button>
                <t-link v-if="quotationNo" theme="primary" class="quotation-link" @click="handleQuotationDetailClick">
                  {{ quotationNo }}
                </t-link>
    ...
    <!-- 报价单选择弹框 -->
    <quotation-select-dialog v-model:visible="productDialogVisible" @confirm="handleQuotationConfirm" />
  </div>
</template>
...
import { addContract, editContract, getContractDetail } from '@/api/contract';
...
import QuotationSelectDialog from './quotationSelectDialog.vue';
...
// 是否为编辑模式
const isEditMode = computed(() => !!props.queryId);
...
// 报价单确认
const handleQuotationConfirm = async (quotation: any) => {
  if (!quotation || !quotation.id) {
    MessagePlugin.warning('未获取到报价单信息');
    return;
  }
...
const handleQuotationDetailClick = () => {
  if (formData.value.quotation_id) {
    router.push({
      name: 'QuotationDetail',
      query: { quotationId: formData.value.quotation_id },
    });
  }
};
...
// 提交合同
const submitContract = async (isApproval: boolean) => {
  try {
    // 处理附件数据，转换为指定格式
    ...
    // 编辑模式多传一个合同id，并调用编辑接口
    let response;
    if (isEditMode.value && props.queryId) {
      submitData.id = props.queryId;
      response = await editContract(submitData);
    } else {
      response = await addContract(submitData);
    }
    ...
    if (response.code === 0 || response.code === 200) {
      ...
      MessagePlugin.error(response.msg || response.message || '操作失败');
    }
  } catch (error: any) {
    console.error('提交合同失败:', error);
    MessagePlugin.error(error?.response?.data?.msg || error?.message || '操作失败，请重试');
  }
};
...
// 根据报价单ID加载报价详情并填充表单与产品信息
const loadQuotationDetail = async (quotationId: string | number) => {
  if (!quotationId) {
  ...
  try {
    ...
    if (response && (response.code === 0 || response.code === 200) && response.data) {
      ...
      // 填充关联报价ID
      formData.value.quotation_id = Number(quotationId);
...
const loadContractDetail = async (id: string) => {
  try {
    ...
    if (res && (res.code === 0 || res.code === 200) && res.data) {
      ...
      // 基础字段填充
      formData.value.contract_title = detail.contract_title || '';
      formData.value.customer_id = detail.customer_id ? Number(detail.customer_id) : '';
      formData.value.quotation_id = detail.quotation_id ? Number(detail.quotation_id) : '';
      formData.value.contract_amount = Number(detail.contract_amount) || 0;
      formData.value.contract_type = detail.contract_type || '';
      formData.value.sign_date = detail.sign_date || '';
      formData.value.maintain_time = detail.maintain_time || '';
      formData.value.delivery_date = detail.delivery_date || '';
      formData.value.customer_signer = detail.customer_signer || '';
      formData.value.our_signer = detail.our_signer || '';
      formData.value.delivery_address = detail.delivery_address || '';

      formData.value.owner_user_id = detail.owner_user_id ? String(detail.owner_user_id) : '';
      formData.value.remark = detail.remark || '';
      // 处理回款计划数据，确保字段匹配
      if (Array.isArray(detail.payment_plan)) {
        paymentStages.value = detail.payment_plan.map((item: any) => {
          // 处理百分比字符串，去除%符号并转换为数字
          let percentage = 0;
          if (item.percentage) {
            const percentageStr = String(item.percentage).replace('%', '');
            percentage = Number(percentageStr) || 0;
          }
          ...
          return {
            ...
            percentage,
            amount,
            stage: item.stage_name || item.stage || '',
          };
        });
      } else {
        paymentStages.value = [];
      }
      console.log('paymentStages.value:', paymentStages.value);
      // 附件还原到上传组件格式
      if (Array.isArray(detail.attachments)) {
        formData.value.attachments = detail.attachments.map((att: any) => ({
      ...
      // 如果有关联的报价单ID，加载报价单详情获取报价单号
      if (detail.quotation_id) {
        try {
          const quotationRes = await getQuotationDetail(detail.quotation_id);
          if (quotationRes && (quotationRes.code === 0 || quotationRes.code === 200) && quotationRes.data) {
            quotationNo.value = quotationRes.data.quotation_no || '';
          }
          // 填充关联报价的商品
          if (
            quotationRes.data.products &&
            Array.isArray(quotationRes.data.products) &&
            quotationRes.data.products.length > 0
          ) {
            productList.value = quotationRes.data.products.map((item: any) => ({
              product_name: item.product_name || '',
              unit: item.unit || '',
              quantity: Number(item.quantity) || 0,
              unit_price: Number(item.unit_price) || 0,
    ...
    MessagePlugin.error(error?.response?.data?.msg || error?.message || '获取合同详情失败，请重试');
  }
};
...
onMounted(() => {
  ...
  // 如果是编辑模式，加载详情
  if (isEditMode.value && props.queryId) {
    loadContractDetail(props.queryId);
  }
});
</script>
